...
console.log(users.filter(({ip_address}) => ip_address.split('.')[0] < 128).length)
...
const position = users.findIndex(({ip_address}) => {
    const first0ctect = ip_address.split('.')[0];
    return first0ctect >= 128 && first0ctect <192;
});
...
const findUserObject = users.find(({email}) => email.includes('soundcloud'));

console.log(findUserObject);
